...
class ImageEditorApp:
    ...
    def apply_brush_to_mask(self, x, y, size, action):
         if action == 'Add':
             new_label = 1  # Use the existing label
         elif action == 'Delete':
             new_label = 0
    ...
    def update_model_mask_on_canvas(self):
        ...
        plt.imshow(self.merged_mask)##Merged mask to be np.unique as before not multiple pixels 
    ...
    def segment_image(self):
        ...
        self.colors = np.random.randint(0, 256, size=(num_masks, 3), dtype=np.uint8)
        ...
        for i in range(num_masks):
            color_mask[merged_mask == i + 1] = self.colors[i]
