...
class ImageEditorApp:
    def __init__(self, root):
        ...
        self.overlaid_mask_canvas.bind("<Motion>", self.draw_brush)#allow brush drawing on top of the overlaid mask -- we can move it directly to rgb later 
    ...
    def paint(self, event): #Brush one time click 
        if self.paint_active:
            ...
            # Create the brush image with transparency
            brush_image = self.create_brush_image(size, brush_color, alpha)
            tk_brush_image = ImageTk.PhotoImage(brush_image)

            # Draw the brush image on the canvas
            self.overlaid_mask_canvas.create_image(x, y, image=tk_brush_image, anchor="center")
            self.overlaid_mask_canvas.image = tk_brush_image  # Keep a reference to avoid garbage collection
