...
from utils import build_sam_vit_b, build_sam_vit_h, build_sam_vit_l, update_sam_model
...
class ImageEditorApp:
    ...
    def draw_brush(self, event):
       if self.brush_active.get() == 1:
           # Clear the previous brush drawing
           if self.brush_id:
           ...
           # Draw the brush as a circle
           self.brush_id = self.overlaid_mask_canvas.create_oval(x - size, y - size, x + size, y + size,
                                                                 outline='red' if action == 'Delete' else 'green',
                                                                 width=2)
           self.overlaid_mask_canvas.lift(self.brush_id)  # Ensure the brush is on top of the image
